#!/usr/bin/env python3
"""
Script to verify key ownership (primary & replica nodes) before and after adding a server.
Useful for manual validation of rebalancing behavior.
"""
...
async def main():
    print("\n" + "="*80)
    print("DISTRIBUTED CACHE REBALANCING VERIFICATION")
    print("="*80)

    # Step 1: Create initial cluster with 2 nodes
    print("\n[1/5] Creating initial cluster with 2 nodes...")
    node_specs = [
    ...
    # Step 3: Print ownership BEFORE adding server
    ownership_before = await print_key_ownership(cluster, test_keys, "BEFORE ADDING NODE")

    # Step 4: Add new node
    print(f"\n[3/5] Adding new node (node-c)...")
    new_node_config = NodeConfig("node-c", "127.0.0.1", 8003)
    new_runtime = NodeRuntime.create(new_node_config, max_items=100)
    # register_node is called inside add_node if it exists
    result = await cluster.add_node(new_node_config, new_runtime)
    if result.get("ok"):
        print(f"  ✓ Added node-c and rebalanced data")
    else:
    ...
    if changed_keys:
        print(f"\nKeys that moved to new primary:")
